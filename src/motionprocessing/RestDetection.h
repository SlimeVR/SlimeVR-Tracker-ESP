...
public:
    ...
    void update6D(uint32_t dtMicros, sensor_real_t acc[3], sensor_real_t gyr[3]) {
        updateAcc(dtMicros, acc);
        updateGyr(dtMicros, gyr); //gyro doesn't set restDection to true only false
    }
